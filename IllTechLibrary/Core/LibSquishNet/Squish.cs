...
namespace Squish
{
    ...
    public static class Squish
    {
        ...
        static void Decompress(byte[] rgba, ref byte[] block, int offset, SquishFlags flags)
        {
            ...
            {
                DecompressAlphaDxt3(rgba, ref block, alphaBlock);
            }
        ...
        // RGBA is output block is input
        static void DecompressAlphaDxt3(byte[] rgba, ref byte[] block, int offset)
        {
            // unpack the alpha values pairwise
            for (int i = 0; i < 8; ++i)
            {
                // quantise down to 4 bits
                byte quant = block[offset+i];
